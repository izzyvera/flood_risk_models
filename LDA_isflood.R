...
# split the data
correlationmatrix = cor(uniondata[-18])
...
data$Flood.type = as.factor(uniondata$Flood.type)
set.seed(3033)
intrain <- createDataPartition(y = data$Flood.type, p= 0.75, list = FALSE)
training <- data[intrain,]
testing <- data[-intrain,]

# ridge regression
# x=model.matrix(Flood.type~.,data)
# y=data$Flood.type
# grid=10^(-2:10)
...
# lda
invalid <- createDataPartition(y = training$Flood.type, p= 0.75, list = FALSE)
train <- training[invalid,]
valid <- data[-invalid,]
lda.fit = lda(train$Flood.type~.,train)
lda.fit
...
y = valid$Flood.type
t = table(lda.class, y)
tpr = (sum(t[6:8]) + sum(t[10:12]) + sum(t[14:16]) ) / (sum(t) - sum(t[1:4])) # 0.572549
mean(lda.class== y) # accuracy 0.9150721
lda2.fit = lda(training$Flood.type~.,training)
lda2.fit
lda2.pred = predict(lda2.fit, testing)
names(lda2.pred)
lda2.class= lda2.pred$class
t2 = table(lda2.class, testing$Flood.type)
tpr2 = (sum(t2[6:8]) + sum(t2[10:12]) + sum(t2[14:16]) ) / (sum(t2) - sum(t2[1:4])) # 0.4611111
...
training <- data[intrain,]
testing <- data[-intrain,]
data$Flood.type = ifelse((data$Flood.type == 1 | data$Flood.type == 2 | data$Flood.type == 3), 1, 0)
set.seed(3033)
intrain <- createDataPartition(y = data$Flood.type, p= 0.75, list = FALSE)
training <- data[intrain,]
testing <- data[-intrain,]
...
y = valid$Flood.type
t = table(lda.class, y)
tpr = t[4] / (t[3] + t[4]) # 0.547619
mean(lda.class== y) # accuracy 0.9507467
lda2.fit = lda(training$Flood.type~.,training)
lda2.fit
lda2.pred = predict(lda2.fit, testing)
names(lda2.pred)
lda2.class= lda2.pred$class
t2 = table(lda2.class, testing$Flood.type)
tpr2 = t2[4] / (t2[3] + t2[4]) #  0.5063291
lda3.pred = predict(lda.fit, testing)
names(lda3.pred)
lda3.class= lda3.pred$class
t3 = table(lda3.class, testing$Flood.type)
tpr3 = t3[4] / (t3[3] + t3[4]) # 0.5063291
...
ldadata = ldadata[-2]
ldadata$Flood.type = ifelse((ldadata$Flood.type == 1 | ldadata$Flood.type == 2 | ldadata$Flood.type == 3), 1, 0)
data = as.data.frame(scale(ldadata))
data$Flood.type = as.factor(ldadata$Flood.type)
set.seed(3033)
intrain <- createDataPartition(y = data$Flood.type, p= 0.75, list = FALSE)
training <- data[intrain,]
testing <- data[-intrain,]
...
LDAmodel$ConfusionMatrixResults
LDAmodel$ConfMatrix
...
data2$Flood.type = as.factor(uniondata$Flood.type)
set.seed(3033)
intrain <- createDataPartition(y = data2$Flood.type, p= 0.75, list = FALSE)
training <- data2[intrain,]
testing <- data2[-intrain,]

lda.fit = lda(training$Flood.type~.,training)
lda.fit
lda.pred = predict(lda.fit, testing)
names(lda.pred)
lda.class= lda.pred$class
t = table(lda.class, testing$Flood.type)
tpr = (sum(t[6:8]) + sum(t[10:12]) + sum(t[14:16]) ) / (sum(t) - sum(t[1:4])) # 0.4722222


data2$Flood.type = ifelse((ldadata$Flood.type == 1 | ldadata$Flood.type == 2 | ldadata$Flood.type == 3), 1, 0)
data2$Flood.type = as.factor(data2$Flood.type)
set.seed(3033)
intrain <- createDataPartition(y = data2$Flood.type, p= 0.75, list = FALSE)
training <- data2[intrain,]
testing <- data2[-intrain,]
...
lda.class= lda.pred$class
t = table(lda.class, testing$Flood.type)
tpr = t[4] / (t[3] + t[4]) # 0.4696133

data2$Flood.type = ifelse((ldadata$Flood.type == 1 | ldadata$Flood.type == 2 | ldadata$Flood.type == 3), 1, 0)
data2$Month = as.factor(uniondata$Month)
set.seed(3033)
intrain <- createDataPartition(y = data2$Flood.type, p= 0.75, list = FALSE)
training <- data2[intrain,]
testing <- data2[-intrain,]
